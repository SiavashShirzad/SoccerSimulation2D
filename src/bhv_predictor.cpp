...
Bhv_Predictor::predictOpponentsReachStep( const rcsc::WorldModel & wm,
					    ...
					    rcsc::Vector2D * interception_pos
					)
{
...
//first_min_step = SP.ballMoveStep( first_ball_vel.r(), first_ball_pos.dist( sol1 ) );
    ...
    rcsc::Vector2D interception_pos_temp;
          ...
          ++o )
    {
        int step = predictOpponentReachStep( *o,
                                             ...
                                             ball_move_angle,
                                             min_step,
					     &interception_pos_temp );
        if ( step < min_step )
        {
            min_step = step;
	    if(interception_pos)
	      *interception_pos = interception_pos_temp;
        }
    }
...
int
Bhv_Predictor::predictTeammatesReachStep( const rcsc::WorldModel & wm,
                                            const rcsc::Vector2D & first_ball_pos,
                                            const rcsc::Vector2D & first_ball_vel,
                                            const rcsc::AngleDeg & ball_move_angle,
					   int & teammate_number,
					...
					)
{
  ...
  //          first_min_step = SP.ballMoveStep( first_ball_vel.r(), first_ball_pos.dist( sol1 ) );
    ...
    int min_step = first_min_step;
    rcsc::Vector2D interception_pos_temp;
    const rcsc::AbstractPlayerCont::const_iterator end = wm.allTeammates().end();
    for ( rcsc::AbstractPlayerCont::const_iterator o = wm.allTeammates().begin();
          o != end;
          ++o )
    {
        int step = predictOpponentReachStep( *o,
 ...
 */
int
Bhv_Predictor::predictTeammateReachStep( const rcsc::AbstractPlayerObject * opponent,
                                           const rcsc::Vector2D & first_ball_pos,
                                           const rcsc::Vector2D & first_ball_vel,
                                           const rcsc::AngleDeg & ball_move_angle,
                                           const int max_cycle,
